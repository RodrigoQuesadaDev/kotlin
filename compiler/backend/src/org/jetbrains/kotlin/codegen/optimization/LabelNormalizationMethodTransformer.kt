...
public class LabelNormalizationMethodTransformer : MethodTransformer() {
    ...
    private class TransformerForMethod(val methodNode: MethodNode) {
        ...
        val removedLabels = hashSetOf<Label>()

        public fun transform() {
            rewriteLabelInstructions()
            if (removedLabels.isEmpty()) return
        ...
        private fun rewriteLabelInstructions() {
            ...
            while (thisNode != null) {
                if (thisNode is LabelNode) {
                    ...
                    if (prevNode is LabelNode) {
                        ...
                        removedLabels.add(thisNode.label)
        ...
        private fun rewriteLocalVars() {
            methodNode.localVariables = methodNode.localVariables.map { oldVar ->
                LocalVariableNode(
                        ...
                        oldVar.desc,
                        oldVar.signature,
                        getNewOrOld(oldVar.start),
                        getNewOrOld(oldVar.end),
                        oldVar.index
                )
            }
        }
