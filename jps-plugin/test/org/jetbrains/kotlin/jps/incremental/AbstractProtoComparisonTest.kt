...
public abstract class AbstractProtoComparisonTest : UsefulTestCase() {
    ...
    public fun doTest(testDataPath: String) {
        ...
        assertEquals(oldClassFiles.size(), newClassFiles.size())
        ...
        UsefulTestCase.assertSameElements("Set of classes are different", oldSetOfNames, newSetOfNames)

        for(i in oldClassFiles.indices) {
            p.printDifference(oldClassFiles[i], newClassFiles[i])
        }
    ...
    private fun compileFileAndGetClasses(testPath: String, testDir: File, fileName: String): List<File> {
        ...
        return File(classesDirectory, "test").listFiles() { it.name.endsWith(".class") && !it.name.contains('$') }?.sortBy { it.name }!!
    }
    ...
    private fun Printer.printDifference(oldClassFile: File, newClassFile: File) {
        ...
        val oldProto = ProtoMapValue(oldClassHeader.isCompatiblePackageFacadeKind(), oldProtoBytes)
        val newProto = ProtoMapValue(newClassHeader.isCompatiblePackageFacadeKind(), newProtoBytes)

        val diff = when {
            newClassHeader.isCompatiblePackageFacadeKind(), newClassHeader.isCompatibleClassKind() ->
